...
import { Icon, type InsIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type InsTab = 'home' | 'protection' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: InsTab; onSelect: (t: InsTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: InsTab; icon: InsIconName; label: string }[] = [
    { key: 'home',       icon: 'shield',   label: t.insure.nav.home       },
    { key: 'protection', icon: 'grid',     label: t.insure.nav.protection },
    { key: 'pro',        icon: 'star',     label: t.insure.nav.pro        },
    { key: 'settings',   icon: 'settings', label: t.insure.nav.settings   },
  ];
  return (
    <nav style={{
